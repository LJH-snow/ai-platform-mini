...
- **naive created_at UTC 规范化** — 将无时区 ISO 时间戳强制解释为 UTC，需独立评估非 UTC 部署兼容性（Sprint 8，已完成）
- **空流 fallback 模型字段** — `chat_completions_stream()` 空流 fallback 应使用已解析的 `model` 而非 `default_model`（Sprint 8，已完成）
