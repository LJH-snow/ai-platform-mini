...
- 未完成项必须保持原样标注：RAG 失败状态已验证，成功来源/空知识库路径未验证；Ollama embedding 服务不可用，后端 `/api/embed` 返回 404；完整屏幕阅读器验证尚未完成；回答内精确引用、持久化 Trace、事件回放和精确 usage 不在本阶段完成。
