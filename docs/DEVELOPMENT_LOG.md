...
### Sprint M17（Workflow Builder 运行快照恢复，2026-09-09）

- `WorkflowBuilder` 运行详情区新增“载入快照到画布”按钮：选中任意历史 Run 后，
  一键把该 Run 的 `definition` 快照通过既有 `definitionToCanvas` 载回画布，
  并把 `inputs` 一起回填到试运行输入框，便于复现或继续调整同一次工作流。
- 若当前画布属于已发布流程，载入快照后提示“发布流程需先取消发布再保存”，
  避免用户误以为已发布状态可以直接原地修改；保存草稿仍走原有发布流程约束。
- `WorkflowBuilder.css` 补充运行详情内的快照按钮布局，保持现有 run facts/timeline
  视觉一致。
- 新增组件测试：打开工作流、选中历史 Run、点击载入，断言提示语和输入 JSON 都按
  快照恢复。

#### Sprint M17 学习总结

运行历史里保存的 definition 是不可变快照，天然适合作为回放/恢复画布的输入源，
